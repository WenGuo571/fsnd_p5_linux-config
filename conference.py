...
"""
Conference Central API implemented using Google Cloud Endpoints.
...
FEATUREDSPEAKER_TPL = ('The conference %s, has just had %s added as the '
                       'featured speaker!')
...
# - - - - - - - - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        if user_id != conf.organizerUserId:
            raise endpoints.ForbiddenException('Only conference owner \
                can create conference sessions.')

        if not request.name:
            raise endpoints.BadRequestException("Session name is required!")

        # add default values for those missing field
        ...
        if data['startTime']:
            try:
                data['startTime'] = datetime.strptime(
                    data['startTime'], "%I:%M%p").time()
            except:
                data['startTime'] = datetime.strptime(
                    data['startTime'], "%H:%M").time()

        # sort highlights list
        if data['highlights']:
            data['highlights'] = data['highlights'].sort()
    ...
    def getSessionsOnDateWithMaxDuration(self, request):
        ...
        filteredSessions2 = filteredSessions.filter(
    ...
    @endpoints.method(SESS_TYPETIME_REQUEST, SessionForms,
                      path='filterSessions/{typeOfSession}/{startTime}',
                      http_method='GET', name='filterSessions')
    def filterSessions(self, request):
        """Filter sessions with latest start time and session type not interested."""
        try:
            filterTime = datetime.strptime(
                urllib2.unquote(request.startTime), "%H:%M").time()
        except:
        ...
        for sess in filteredSessions:
            if sess.startTime is not None and sess.startTime <= filterTime:
...
# - - - Session Wishlists - - - - - - - - - - - - - - - - - - - -
    ...
    def _sessionWishlist(self, request, save=True):
        ...
        # check if websafeSessionKey in wishlist
        ...
        if save:
            ...
            if wssk in prof.sessionKeysOnWishlist:
                raise ConflictException(
                    "Session is already on wishlist!")
            # register user, take away one seat
...
# - - - Featured Speaker - - - - - - - - - - - - - - - - - - - -
    ...
    def _cacheFeaturedSpeaker(conference, featuredSpeaker):
        ...
        announcement = FEATUREDSPEAKER_TPL.format(conference, featuredSpeaker)
